//===-- DummyLLParser.h - Parser Class -------------------------------*- C++
//-*-===//
//
...
namespace llvm {
  ...
  public:
    DummyLLParser(StringRef F, SourceMgr &SM, SMDiagnostic &Err, Module *M,
                  ModuleSummaryIndex *Index, LLVMContext &Context,
                  bool TypeCheck = true, SlotMapping *Slots = nullptr,
                  bool UpgradeDebugInfo = true, StringRef DataLayoutString = "")
        : Context(Context), Lex(F, SM, Err, Context), M(M), Index(Index),
          TypeCheck(TypeCheck), Slots(Slots), BlockAddressPFS(nullptr),
  ...
  private:
    ...
    class PerFunctionState {
      ...
      int FunctionNumber;
    public:
      PerFunctionState(DummyLLParser &p, Function &f, int functionNumber);
      ~PerFunctionState();
