//===-- DummyLLParser.cpp - Parser Class
//---------------------------------------===//
//
...
bool DummyLLParser::parseStandaloneConstantValue(Constant *&C,
...
bool DummyLLParser::parseTypeAtBeginning(Type *&Ty, unsigned &Read,
                                         const SlotMapping *Slots) {
  restoreParsingState(Slots);
  Lex.Lex();
...
void DummyLLParser::restoreParsingState(const SlotMapping *Slots) {
  if (!Slots)
    return;
...
/// module.
bool DummyLLParser::ValidateEndOfModule() {
  if (!M)
    return false;
  ...
  // the mapping from DummyLLParser as it doesn't need it anymore.
...
bool DummyLLParser::ValidateEndOfIndex() {
  if (!Index)
    return false;
...
bool DummyLLParser::ParseTopLevelEntities() {
...
bool DummyLLParser::ParseModuleAsm() {
...
bool DummyLLParser::ParseTargetDefinition() {
...
bool DummyLLParser::ParseSourceFileName() {
...
bool DummyLLParser::ParseDepLibs() {
...
bool DummyLLParser::ParseUnnamedType() {
...
/// toplevelentity
///   ::= LocalVar '=' 'type' type
bool DummyLLParser::ParseNamedType() {
...
bool DummyLLParser::ParseDeclare() {
...
bool DummyLLParser::ParseDefine() {
...
///   ::= 'constant'
///   ::= 'global'
bool DummyLLParser::ParseGlobalType(bool &IsConstant) {
...
bool DummyLLParser::ParseOptionalUnnamedAddr(
...
///                OptionalDLLStorageClass
///                                                     ...   -> global variable
bool DummyLLParser::ParseUnnamedGlobal() {
...
///                                                     ...   -> global variable
bool DummyLLParser::ParseNamedGlobal() {
...
bool DummyLLParser::parseComdat() {
...
// MDString:
//   ::= '!' STRINGCONSTANT
bool DummyLLParser::ParseMDString(MDString *&Result) {
  std::string Str;
  if (ParseStringConstant(Str)) return true;
...
// MDNode:
//   ::= '!' MDNodeNumber
bool DummyLLParser::ParseMDNodeID(MDNode *&Result) {
  // !{ ..., !42, ... }
...
/// ParseNamedMetadata:
///   !foo = !{ !1, !2 }
bool DummyLLParser::ParseNamedMetadata() {
...
///   !42 = !{...}
bool DummyLLParser::ParseStandaloneMetadata() {
...
bool DummyLLParser::SkipModuleSummaryEntry() {
...
bool DummyLLParser::ParseSummaryEntry() {
...
///
bool DummyLLParser::parseIndirectSymbol(
  ...
  if (!Name.empty()) {
    ...
    if (GVal) {
      if (TypeCheck && !ForwardRefVals.erase(Name))
...
/// already.
///
bool DummyLLParser::ParseGlobal(const std::string &Name, LocTy NameLoc,
...
bool DummyLLParser::ParseUnnamedAttrGrp() {
...
///   ::= <attr> | <attr> '=' <value>
bool DummyLLParser::ParseFnAttributeValuePairs(
...
Value *DummyLLParser::checkValidVariableType(LocTy Loc, const Twine &Name,
                                             Type *Ty, Value *Val,
                                             bool IsCall) {
  return Val;
  /*  if (Val->getType() == Ty)
...
GlobalValue *DummyLLParser::GetGlobalVal(const std::string &Name, Type *Ty,
...
GlobalValue *DummyLLParser::GetGlobalVal(unsigned ID, Type *Ty, LocTy Loc,
                                         bool IsCall) {
  PointerType *PTy = dyn_cast<PointerType>(Ty);
  if (!PTy) {
    Error(Loc, "global variable reference must have pointer type");
...
Comdat *DummyLLParser::getComdat(const std::string &Name, LocTy Loc) {
...
bool DummyLLParser::ParseToken(lltok::Kind T, const char *ErrMsg) {
  if (Lex.getKind() != T)
...
/// ParseStringConstant
///   ::= StringConstant
bool DummyLLParser::ParseStringConstant(std::string &Result) {
...
/// ParseUInt32
///   ::= uint32
bool DummyLLParser::ParseUInt32(uint32_t &Val) {
...
/// ParseUInt64
///   ::= uint64
bool DummyLLParser::ParseUInt64(uint64_t &Val) {
  if (Lex.getKind() != lltok::APSInt || Lex.getAPSIntVal().isSigned())
    return TokError("expected integer");
...
///   := 'initialexec'
///   := 'localexec'
bool DummyLLParser::ParseTLSModel(GlobalVariable::ThreadLocalMode &TLM) {
  switch (Lex.getKind()) {
    default:
...
bool DummyLLParser::ParseOptionalThreadLocal(
    GlobalVariable::ThreadLocalMode &TLM) {
...
///   := /*empty*/
///   := 'addrspace' '(' uint32 ')'
bool DummyLLParser::ParseOptionalAddrSpace(unsigned &AddrSpace,
                                           unsigned DefaultAS) {
  AddrSpace = DefaultAS;
...
bool DummyLLParser::ParseStringAttribute(AttrBuilder &B) {
...
bool DummyLLParser::ParseOptionalParamAttrs(AttrBuilder &B) {
  bool HaveError = false;
...
bool DummyLLParser::ParseOptionalReturnAttrs(AttrBuilder &B) {
  bool HaveError = false;
...
///   ::= 'extern_weak'
///   ::= 'external'
bool DummyLLParser::ParseOptionalLinkage(unsigned &Res, bool &HasLinkage,
                                         unsigned &Visibility,
...
void DummyLLParser::ParseOptionalDSOLocal(bool &DSOLocal) {
  switch (Lex.getKind()) {
  default:
    DSOLocal = false;
    break;
...
///   ::= 'protected'
///
void DummyLLParser::ParseOptionalVisibility(unsigned &Res) {
  switch (Lex.getKind()) {
  default:
...
///   ::= 'dllexport'
///
void DummyLLParser::ParseOptionalDLLStorageClass(unsigned &Res) {
  switch (Lex.getKind()) {
  default:
...
///   ::= 'cc' UINT
///
bool DummyLLParser::ParseOptionalCallingConv(unsigned &CC) {
  switch (Lex.getKind()) {
...
///   ::= !dbg !42
bool DummyLLParser::ParseMetadataAttachment(unsigned &Kind, MDNode *&MD) {
...
///   ::= !dbg !42 (',' !dbg !57)*
bool DummyLLParser::ParseInstructionMetadata(Instruction &Inst) {
  do {
    if (Lex.getKind() != lltok::MetadataVar)
...
///   ::= !dbg !57
bool DummyLLParser::ParseGlobalObjectMetadataAttachment(GlobalObject &GO) {
  unsigned MDK;
  MDNode *N;
  if (ParseMetadataAttachment(MDK, N))
    return true;
...
///   ::= (!dbg !57)*
bool DummyLLParser::ParseOptionalFunctionMetadata(Function &F) {
  while (Lex.getKind() == lltok::MetadataVar)
...
///   ::= /* empty */
///   ::= 'align' 4
bool DummyLLParser::ParseOptionalAlignment(unsigned &Alignment) {
  Alignment = 0;
...
bool DummyLLParser::ParseOptionalDerefAttrBytes(lltok::Kind AttrKind,
                                                uint64_t &Bytes) {
...
/// end.
bool DummyLLParser::ParseOptionalCommaAlign(unsigned &Alignment,
                                            bool &AteExtraComma) {
  AteExtraComma = false;
  while (EatIfPresent(lltok::comma)) {
...
/// This returns with AteExtraComma set to true if it ate an excess comma at the
/// end.
bool DummyLLParser::ParseOptionalCommaAddrSpace(unsigned &AddrSpace, LocTy &Loc,
                                                bool &AteExtraComma) {
  AteExtraComma = false;
  while (EatIfPresent(lltok::comma)) {
    // Metadata at the end is an early exit.
...
bool DummyLLParser::parseAllocSizeArguments(unsigned &BaseSizeArg,
...
bool DummyLLParser::ParseScopeAndOrdering(bool isAtomic, SyncScope::ID &SSID,
...
bool DummyLLParser::ParseScope(SyncScope::ID &SSID) {
...
bool DummyLLParser::ParseOrdering(AtomicOrdering &Ordering) {
  switch (Lex.getKind()) {
...
///   ::= /* empty */
///   ::= 'alignstack' '(' 4 ')'
bool DummyLLParser::ParseOptionalStackAlignment(unsigned &Alignment) {
  Alignment = 0;
...
///    ::=  (',' uint32)+
///
bool DummyLLParser::ParseIndexList(SmallVectorImpl<unsigned> &Indices,
                                   bool &AteExtraComma) {
  AteExtraComma = false;
...
bool DummyLLParser::ParseType(Type *&Result, const Twine &Msg, bool AllowVoid) {
...
bool DummyLLParser::ParseParameterList(SmallVectorImpl<ParamInfo> &ArgList,
...
bool DummyLLParser::ParseOptionalOperandBundles(
...
///   ::= ArgType (',' ArgType)*
///
bool DummyLLParser::ParseArgumentList(SmallVectorImpl<ArgInfo> &ArgList,
                                      bool &isVarArg) {
  isVarArg = false;
...
bool DummyLLParser::ParseFunctionType(Type *&Result) {
  assert(Lex.getKind() == lltok::lparen);
...
/// other structs.
bool DummyLLParser::ParseAnonStructType(Type *&Result, bool Packed) {
...
bool DummyLLParser::ParseStructDefinition(SMLoc TypeLoc, StringRef Name,
...
///     ::= '<' '{' '}' '>'
///     ::= '<' '{' Type (',' Type)* '}' '>'
bool DummyLLParser::ParseStructBody(SmallVectorImpl<Type *> &Body) {
...
///     ::= '[' APSINTVAL 'x' Types ']'
///     ::= '<' APSINTVAL 'x' Types '>'
bool DummyLLParser::ParseArrayVectorType(Type *&Result, bool isVector) {
  if (Lex.getKind() != lltok::APSInt || Lex.getAPSIntVal().isSigned() ||
...
DummyLLParser::PerFunctionState::PerFunctionState(DummyLLParser &p, Function &f,
                                                  int functionNumber)
...
DummyLLParser::PerFunctionState::~PerFunctionState() {
...
bool DummyLLParser::PerFunctionState::FinishFunction() {
  if (!ForwardRefVals.empty())
...
/// forward reference record if needed.  This can return null if the value
/// exists but does not have the right type.
Value *DummyLLParser::PerFunctionState::GetVal(const std::string &Name,
                                               Type *Ty, LocTy Loc,
                                               bool IsCall) {
  // Look this name up in the normal function symbol table.
...
Value *DummyLLParser::PerFunctionState::GetVal(unsigned ID, Type *Ty, LocTy Loc,
                                               bool IsCall) {
  // Look this name up in the normal function symbol table.
...
bool DummyLLParser::PerFunctionState::SetInstName(int NameID,
...
BasicBlock *DummyLLParser::PerFunctionState::GetBB(const std::string &Name,
                                                   LocTy Loc) {
  return dyn_cast_or_null<BasicBlock>(
      GetVal(Name, Type::getLabelTy(F.getContext()), Loc, /*IsCall=*/false));
}

BasicBlock *DummyLLParser::PerFunctionState::GetBB(unsigned ID, LocTy Loc) {
  return dyn_cast_or_null<BasicBlock>(
...
BasicBlock *DummyLLParser::PerFunctionState::DefineBB(const std::string &Name,
                                                      LocTy Loc) {
  BasicBlock *BB;
  if (Name.empty())
...
bool DummyLLParser::ParseValID(ValID &ID, PerFunctionState *PFS) {
  ID.Loc = Lex.getLoc();
  switch (Lex.getKind()) {
  ...
  case lltok::kw_select: {
    ...
    if (Opc == Instruction::GetElementPtr) {
      ...
      if (TypeCheck && Ty != BasePointerType->getElementType())
        return Error(
            ExplicitTypeLoc,
            "explicit pointee type doesn't match operand's pointee type");
...
bool DummyLLParser::ParseGlobalValue(Type *Ty, Constant *&C) {
  C = nullptr;
  ValID ID;
  Value *V = nullptr;
...
bool DummyLLParser::ParseGlobalTypeAndValue(Constant *&V) {
  Type *Ty = nullptr;
  return ParseType(Ty) ||
         ParseGlobalValue(Ty, V);
}

bool DummyLLParser::parseOptionalComdat(StringRef GlobalName, Comdat *&C) {
  C = nullptr;
...
bool DummyLLParser::ParseGlobalValueVector(SmallVectorImpl<Constant *> &Elts,
                                           Optional<unsigned> *InRangeOp) {
  // Empty list.
  if (Lex.getKind() == lltok::rbrace ||
...
bool DummyLLParser::ParseMDTuple(MDNode *&MD, bool IsDistinct) {
...
///  ::= !7
///  ::= !DILocation(...)
bool DummyLLParser::ParseMDNode(MDNode *&N) {
  if (Lex.getKind() == lltok::MetadataVar)
...
bool DummyLLParser::ParseMDNodeTail(MDNode *&N) {
  // !{ ... }
  if (Lex.getKind() == lltok::lbrace)
...
template <>
bool DummyLLParser::ParseMDField(LocTy Loc, StringRef Name,
...
template <>
bool DummyLLParser::ParseMDField(LocTy Loc, StringRef Name, LineField &Result) {
...
template <>
bool DummyLLParser::ParseMDField(LocTy Loc, StringRef Name, MDField &Result) {
...
bool DummyLLParser::ParseMDFieldsImplBody(ParserTy parseField) {
  do {
...
template <class ParserTy>
bool DummyLLParser::ParseMDFieldsImpl(ParserTy parseField, LocTy &ClosingLoc) {
...
bool DummyLLParser::ParseMDField(StringRef Name, FieldTy &Result) {
  if (Result.Seen)
...
bool DummyLLParser::ParseSpecializedMDNode(MDNode *&N, bool IsDistinct) {
  assert(Lex.getKind() == lltok::MetadataVar && "Expected metadata type name");
...
///   isImplicitCode: true)
bool DummyLLParser::ParseDILocation(MDNode *&Result, bool IsDistinct) {
...
bool DummyLLParser::ParseGenericDINode(MDNode *&Result, bool IsDistinct) {
#define VISIT_MD_FIELDS(OPTIONAL, REQUIRED)                                    \
...
bool DummyLLParser::ParseDISubrange(MDNode *&Result, bool IsDistinct) {
#define VISIT_MD_FIELDS(OPTIONAL, REQUIRED)                                    \
...
bool DummyLLParser::ParseDIEnumerator(MDNode *&Result, bool IsDistinct) {
#define VISIT_MD_FIELDS(OPTIONAL, REQUIRED)                                    \
...
bool DummyLLParser::ParseDIBasicType(MDNode *&Result, bool IsDistinct) {
#define VISIT_MD_FIELDS(OPTIONAL, REQUIRED)                                    \
...
///                      dwarfAddressSpace: 3)
bool DummyLLParser::ParseDIDerivedType(MDNode *&Result, bool IsDistinct) {
#define VISIT_MD_FIELDS(OPTIONAL, REQUIRED)                                    \
  REQUIRED(tag, DwarfTagField, );                                              \
  OPTIONAL(name, MDStringField, );                                             \
...
bool DummyLLParser::ParseDICompositeType(MDNode *&Result, bool IsDistinct) {
#define VISIT_MD_FIELDS(OPTIONAL, REQUIRED)                                    \
  REQUIRED(tag, DwarfTagField, );                                              \
  OPTIONAL(name, MDStringField, );                                             \
  OPTIONAL(file, MDField, );                                                   \
...
bool DummyLLParser::ParseDISubroutineType(MDNode *&Result, bool IsDistinct) {
#define VISIT_MD_FIELDS(OPTIONAL, REQUIRED)                                    \
  OPTIONAL(flags, DIFlagField, );                                              \
...
bool DummyLLParser::ParseDIFile(MDNode *&Result, bool IsDistinct) {
...
bool DummyLLParser::ParseDICompileUnit(MDNode *&Result, bool IsDistinct) {
  if (!IsDistinct)
...
bool DummyLLParser::ParseDISubprogram(MDNode *&Result, bool IsDistinct) {
  auto Loc = Lex.getLoc();
#define VISIT_MD_FIELDS(OPTIONAL, REQUIRED)                                    \
  OPTIONAL(scope, MDField, );                                                  \
  OPTIONAL(name, MDStringField, );                                             \
...
bool DummyLLParser::ParseDILexicalBlock(MDNode *&Result, bool IsDistinct) {
#define VISIT_MD_FIELDS(OPTIONAL, REQUIRED)                                    \
  REQUIRED(scope, MDField, (/* AllowNull */ false));                           \
  OPTIONAL(file, MDField, );                                                   \
  OPTIONAL(line, LineField, );                                                 \
...
bool DummyLLParser::ParseDILexicalBlockFile(MDNode *&Result, bool IsDistinct) {
#define VISIT_MD_FIELDS(OPTIONAL, REQUIRED)                                    \
  REQUIRED(scope, MDField, (/* AllowNull */ false));                           \
  OPTIONAL(file, MDField, );                                                   \
...
bool DummyLLParser::ParseDINamespace(MDNode *&Result, bool IsDistinct) {
#define VISIT_MD_FIELDS(OPTIONAL, REQUIRED)                                    \
...
bool DummyLLParser::ParseDIMacro(MDNode *&Result, bool IsDistinct) {
#define VISIT_MD_FIELDS(OPTIONAL, REQUIRED)                                    \
...
bool DummyLLParser::ParseDIMacroFile(MDNode *&Result, bool IsDistinct) {
#define VISIT_MD_FIELDS(OPTIONAL, REQUIRED)                                    \
...
bool DummyLLParser::ParseDIModule(MDNode *&Result, bool IsDistinct) {
#define VISIT_MD_FIELDS(OPTIONAL, REQUIRED)                                    \
  REQUIRED(scope, MDField, );                                                  \
  REQUIRED(name, MDStringField, );                                             \
...
bool DummyLLParser::ParseDITemplateTypeParameter(MDNode *&Result,
                                                 bool IsDistinct) {
#define VISIT_MD_FIELDS(OPTIONAL, REQUIRED)                                    \
  OPTIONAL(name, MDStringField, );                                             \
...
bool DummyLLParser::ParseDITemplateValueParameter(MDNode *&Result,
                                                  bool IsDistinct) {
#define VISIT_MD_FIELDS(OPTIONAL, REQUIRED)                                    \
...
bool DummyLLParser::ParseDIGlobalVariable(MDNode *&Result, bool IsDistinct) {
#define VISIT_MD_FIELDS(OPTIONAL, REQUIRED)                                    \
...
///                        file: !1, line: 7, type: !2, arg: 2, flags: 7,
///                        align: 8)
bool DummyLLParser::ParseDILocalVariable(MDNode *&Result, bool IsDistinct) {
#define VISIT_MD_FIELDS(OPTIONAL, REQUIRED)                                    \
  REQUIRED(scope, MDField, (/* AllowNull */ false));                           \
  OPTIONAL(name, MDStringField, );                                             \
...
bool DummyLLParser::ParseDILabel(MDNode *&Result, bool IsDistinct) {
#define VISIT_MD_FIELDS(OPTIONAL, REQUIRED)                                    \
  REQUIRED(scope, MDField, (/* AllowNull */ false));                           \
  REQUIRED(name, MDStringField, );                                             \
  REQUIRED(file, MDField, );                                                   \
...
/// ParseDIExpression:
///   ::= !DIExpression(0, 7, -1)
bool DummyLLParser::ParseDIExpression(MDNode *&Result, bool IsDistinct) {
  assert(Lex.getKind() == lltok::MetadataVar && "Expected metadata type name");
  Lex.Lex();
...
bool DummyLLParser::ParseDIGlobalVariableExpression(MDNode *&Result,
                                                    bool IsDistinct) {
#define VISIT_MD_FIELDS(OPTIONAL, REQUIRED)                                    \
...
bool DummyLLParser::ParseDIObjCProperty(MDNode *&Result, bool IsDistinct) {
#define VISIT_MD_FIELDS(OPTIONAL, REQUIRED)                                    \
  OPTIONAL(name, MDStringField, );                                             \
  OPTIONAL(file, MDField, );                                                   \
  OPTIONAL(line, LineField, );                                                 \
...
///                         line: 7, name: "foo")
bool DummyLLParser::ParseDIImportedEntity(MDNode *&Result, bool IsDistinct) {
#define VISIT_MD_FIELDS(OPTIONAL, REQUIRED)                                    \
  REQUIRED(tag, DwarfTagField, );                                              \
  REQUIRED(scope, MDField, );                                                  \
...
///  ::= metadata !{...}
///  ::= metadata !"string"
bool DummyLLParser::ParseMetadataAsValue(Value *&V, PerFunctionState &PFS) {
...
///  ::= i32 @global
///  ::= i32 7
bool DummyLLParser::ParseValueAsMetadata(Metadata *&MD, const Twine &TypeMsg,
                                         PerFunctionState *PFS) {
  Type *Ty;
  LocTy Loc;
...
///  ::= !"string"
///  ::= !DILocation(...)
bool DummyLLParser::ParseMetadata(Metadata *&MD, PerFunctionState *PFS) {
  if (Lex.getKind() == lltok::MetadataVar) {
    MDNode *N;
...
bool DummyLLParser::ConvertValIDToValue(Type *Ty, ValID &ID, Value *&V,
...
bool DummyLLParser::parseConstantValue(Type *Ty, Constant *&C) {
  C = nullptr;
  ValID ID;
  auto Loc = Lex.getLoc();
...
bool DummyLLParser::ParseValue(Type *Ty, Value *&V, PerFunctionState *PFS) {
  V = nullptr;
  ValID ID;
...
bool DummyLLParser::ParseTypeAndValue(Value *&V, PerFunctionState *PFS) {
  Type *Ty = nullptr;
  return ParseType(Ty) ||
         ParseValue(Ty, V, PFS);
}

bool DummyLLParser::ParseTypeAndBasicBlock(BasicBlock *&BB, LocTy &Loc,
                                           PerFunctionState &PFS) {
  Value *V;
  Loc = Lex.getLoc();
...
bool DummyLLParser::ParseFunctionHeader(Function *&Fn, bool isDefine) {
  // Parse the linkage.
...
bool DummyLLParser::PerFunctionState::resolveForwardRefBlockAddresses() {
  ValID ID;
  if (FunctionNumber == -1) {
    ID.Kind = ValID::t_GlobalName;
    ID.StrVal = F.getName();
...
bool DummyLLParser::ParseFunctionBody(Function &Fn) {
  if (Lex.getKind() != lltok::lbrace)
...
bool DummyLLParser::ParseBasicBlock(PerFunctionState &PFS) {
...
///
int DummyLLParser::ParseInstruction(Instruction *&Inst, BasicBlock *BB,
                                    PerFunctionState &PFS) {
  lltok::Kind Token = Lex.getKind();
  if (Token == lltok::Eof)
...
bool DummyLLParser::ParseCmpPredicate(unsigned &P, unsigned Opc) {
  if (Opc == Instruction::FCmp) {
    switch (Lex.getKind()) {
...
bool DummyLLParser::ParseRet(Instruction *&Inst, BasicBlock *BB,
                             PerFunctionState &PFS) {
  SMLoc TypeLoc = Lex.getLoc();
  Type *Ty = nullptr;
...
bool DummyLLParser::ParseBr(Instruction *&Inst, PerFunctionState &PFS) {
  LocTy Loc, Loc2;
  Value *Op0;
  BasicBlock *Op1, *Op2;
  ...
  if (TypeCheck && Op0->getType() != Type::getInt1Ty(Context))
...
bool DummyLLParser::ParseSwitch(Instruction *&Inst, PerFunctionState &PFS) {
  LocTy CondLoc, BBLoc;
  Value *Cond;
...
bool DummyLLParser::ParseIndirectBr(Instruction *&Inst, PerFunctionState &PFS) {
  LocTy AddrLoc;
  Value *Address;
...
bool DummyLLParser::ParseInvoke(Instruction *&Inst, PerFunctionState &PFS) {
...
/// ParseResume
///   ::= 'resume' TypeAndValue
bool DummyLLParser::ParseResume(Instruction *&Inst, PerFunctionState &PFS) {
...
bool DummyLLParser::ParseExceptionArgs(SmallVectorImpl<Value *> &Args,
                                       PerFunctionState &PFS) {
...
bool DummyLLParser::ParseCleanupRet(Instruction *&Inst, PerFunctionState &PFS) {
...
bool DummyLLParser::ParseCatchRet(Instruction *&Inst, PerFunctionState &PFS) {
...
bool DummyLLParser::ParseCatchSwitch(Instruction *&Inst,
                                     PerFunctionState &PFS) {
  Value *ParentPad;
...
bool DummyLLParser::ParseCatchPad(Instruction *&Inst, PerFunctionState &PFS) {
...
bool DummyLLParser::ParseCleanupPad(Instruction *&Inst, PerFunctionState &PFS) {
...
bool DummyLLParser::ParseUnaryOp(Instruction *&Inst, PerFunctionState &PFS,
...
/// If OperandType is 0, then any FP or integer operand is allowed.  If it is 1,
/// then any integer operand is allowed, if it is 2, any fp operand is allowed.
bool DummyLLParser::ParseArithmetic(Instruction *&Inst, PerFunctionState &PFS,
                                    unsigned Opc, unsigned OperandType) {
...
/// ParseLogical
///  ::= ArithmeticOps TypeAndValue ',' Value {
bool DummyLLParser::ParseLogical(Instruction *&Inst, PerFunctionState &PFS,
                                 unsigned Opc) {
  LocTy Loc; Value *LHS, *RHS;
  if (ParseTypeAndValue(LHS, Loc, PFS) ||
...
bool DummyLLParser::ParseCompare(Instruction *&Inst, PerFunctionState &PFS,
                                 unsigned Opc) {
...
bool DummyLLParser::ParseCast(Instruction *&Inst, PerFunctionState &PFS,
                              unsigned Opc) {
  LocTy Loc;
  Value *Op;
  Type *DestTy = nullptr;
...
bool DummyLLParser::ParseSelect(Instruction *&Inst, PerFunctionState &PFS) {
  LocTy Loc;
  Value *Op0, *Op1, *Op2;
...
bool DummyLLParser::ParseVA_Arg(Instruction *&Inst, PerFunctionState &PFS) {
  Value *Op;
  Type *EltTy = nullptr;
  LocTy TypeLoc;
...
bool DummyLLParser::ParseExtractElement(Instruction *&Inst,
                                        PerFunctionState &PFS) {
  LocTy Loc;
  Value *Op0, *Op1;
  if (ParseTypeAndValue(Op0, Loc, PFS) ||
...
bool DummyLLParser::ParseInsertElement(Instruction *&Inst,
                                       PerFunctionState &PFS) {
  LocTy Loc;
  Value *Op0, *Op1, *Op2;
  if (ParseTypeAndValue(Op0, Loc, PFS) ||
...
bool DummyLLParser::ParseShuffleVector(Instruction *&Inst,
                                       PerFunctionState &PFS) {
  LocTy Loc;
  Value *Op0, *Op1, *Op2;
  if (ParseTypeAndValue(Op0, Loc, PFS) ||
...
int DummyLLParser::ParsePHI(Instruction *&Inst, PerFunctionState &PFS) {
...
bool DummyLLParser::ParseLandingPad(Instruction *&Inst, PerFunctionState &PFS) {
...
///           OptionalAttrs Type Value ParameterList OptionalAttrs
bool DummyLLParser::ParseCall(Instruction *&Inst, PerFunctionState &PFS,
...
int DummyLLParser::ParseAlloc(Instruction *&Inst, PerFunctionState &PFS) {
  Value *Size = nullptr;
...
int DummyLLParser::ParseLoad(Instruction *&Inst, PerFunctionState &PFS) {
  Value *Val; LocTy Loc;
  unsigned Alignment = 0;
  bool AteExtraComma = false;
  bool isAtomic = false;
  ...
  if (TypeCheck && Ty != cast<PointerType>(Val->getType())->getElementType())
...
///       'singlethread'? AtomicOrdering (',' 'align' i32)?
int DummyLLParser::ParseStore(Instruction *&Inst, PerFunctionState &PFS) {
...
int DummyLLParser::ParseCmpXchg(Instruction *&Inst, PerFunctionState &PFS) {
...
int DummyLLParser::ParseAtomicRMW(Instruction *&Inst, PerFunctionState &PFS) {
...
int DummyLLParser::ParseFence(Instruction *&Inst, PerFunctionState &PFS) {
  AtomicOrdering Ordering = AtomicOrdering::NotAtomic;
  SyncScope::ID SSID = SyncScope::System;
...
int DummyLLParser::ParseGetElementPtr(Instruction *&Inst,
                                      PerFunctionState &PFS) {
  Value *Ptr = nullptr;
  Value *Val = nullptr;
  LocTy Loc, EltLoc;
...
int DummyLLParser::ParseExtractValue(Instruction *&Inst,
                                     PerFunctionState &PFS) {
  Value *Val; LocTy Loc;
  SmallVector<unsigned, 4> Indices;
  bool AteExtraComma;
...
int DummyLLParser::ParseInsertValue(Instruction *&Inst, PerFunctionState &PFS) {
...
/// Element
///   ::= 'null' | TypeAndValue
bool DummyLLParser::ParseMDNodeVector(SmallVectorImpl<Metadata *> &Elts) {
...
//===----------------------------------------------------------------------===//
bool DummyLLParser::sortUseListOrder(Value *V, ArrayRef<unsigned> Indexes,
                                     SMLoc Loc) {
  if (V->use_empty())
...
///   ::= '{' uint32 (',' uint32)+ '}'
bool DummyLLParser::ParseUseListOrderIndexes(
...
bool DummyLLParser::ParseUseListOrder(PerFunctionState *PFS) {
  SMLoc Loc = Lex.getLoc();
...
bool DummyLLParser::ParseUseListOrderBB() {
...
bool DummyLLParser::ParseModuleEntry(unsigned ID) {
  assert(Lex.getKind() == lltok::kw_module);
  Lex.Lex();
...
bool DummyLLParser::ParseTypeIdEntry(unsigned ID) {
...
bool DummyLLParser::ParseTypeIdSummary(TypeIdSummary &TIS) {
...
///         [',' 'inlinesBits' ':' UInt64]? ')'
bool DummyLLParser::ParseTypeTestResolution(TypeTestResolution &TTRes) {
...
bool DummyLLParser::ParseOptionalWpdResolutions(
...
///         [',' OptionalResByArg]? ')'
bool DummyLLParser::ParseWpdRes(WholeProgramDevirtResolution &WPDRes) {
...
///                [',' 'bit' ':' UInt32]? ')'
bool DummyLLParser::ParseOptionalResByArg(
...
/// OptionalResByArg
///   ::= 'args' ':' '(' UInt64[, UInt64]* ')'
bool DummyLLParser::ParseArgs(std::vector<uint64_t> &Args) {
...
void DummyLLParser::AddGlobalValueToIndex(
...
bool DummyLLParser::ParseGVEntry(unsigned ID) {
...
bool DummyLLParser::ParseFunctionSummary(std::string Name,
                                         GlobalValue::GUID GUID, unsigned ID) {
...
///         [',' OptionalRefs]? ')'
bool DummyLLParser::ParseVariableSummary(std::string Name,
                                         GlobalValue::GUID GUID, unsigned ID) {
...
///         'aliasee' ':' GVReference ')'
bool DummyLLParser::ParseAliasSummary(std::string Name, GlobalValue::GUID GUID,
                                      unsigned ID) {
...
/// Flag
///   ::= [0|1]
bool DummyLLParser::ParseFlag(unsigned &Val) {
  if (Lex.getKind() != lltok::APSInt || Lex.getAPSIntVal().isSigned())
    return TokError("expected integer");
...
///        [',' 'noInline' ':' Flag]? ')'
bool DummyLLParser::ParseOptionalFFlags(FunctionSummary::FFlags &FFlags) {
...
bool DummyLLParser::ParseOptionalCalls(
    std::vector<FunctionSummary::EdgeTy> &Calls) {
...
bool DummyLLParser::ParseHotness(CalleeInfo::HotnessType &Hotness) {
  switch (Lex.getKind()) {
...
bool DummyLLParser::ParseOptionalRefs(std::vector<ValueInfo> &Refs) {
...
bool DummyLLParser::ParseOptionalTypeIdInfo(
    FunctionSummary::TypeIdInfo &TypeIdInfo) {
...
///         [',' (SummaryID | UInt64)]* ')'
bool DummyLLParser::ParseTypeTests(std::vector<GlobalValue::GUID> &TypeTests) {
...
bool DummyLLParser::ParseVFuncIdList(
...
bool DummyLLParser::ParseConstVCallList(
    lltok::Kind Kind,
...
/// ConstVCall
///   ::= '(' VFuncId ',' Args ')'
bool DummyLLParser::ParseConstVCall(FunctionSummary::ConstVCall &ConstVCall,
                                    IdToIndexMapType &IdToIndexMap,
                                    unsigned Index) {
  if (ParseToken(lltok::lparen, "expected '(' here") ||
...
///         'offset' ':' UInt64 ')'
bool DummyLLParser::ParseVFuncId(FunctionSummary::VFuncId &VFuncId,
                                 IdToIndexMapType &IdToIndexMap,
                                 unsigned Index) {
...
///         'dsoLocal' ':' Flag ')'
bool DummyLLParser::ParseGVFlags(GlobalValueSummary::GVFlags &GVFlags) {
...
bool DummyLLParser::ParseGVarFlags(GlobalVarSummary::GVarFlags &GVarFlags) {
...
/// ModuleReference
///   ::= 'module' ':' UInt
bool DummyLLParser::ParseModuleReference(StringRef &ModulePath) {
  // Parse module id.
...
/// GVReference
///   ::= SummaryID
bool DummyLLParser::ParseGVReference(ValueInfo &VI, unsigned &GVId) {
